...
**The `:` line.** `←` `→` `Home` `End` move the caret, `↑` `↓` walk the lines run this session (the ones that failed too), and `Tab` finishes a command name at the start of the line or a path anywhere else — **as far as the candidates agree**, listing them when there is more than one rather than choosing for you. `Ctrl+U` clears the line, `Ctrl+V` pastes.
