...
-- 記号。**既定 false**（2026-09-11 に変えました。持っていない端末のほうが
-- 多いためです）。入っている端末では true にすると出ます。窓版は書体を
-- 同梱しているので、この設定を読みません。
...
--  スニペット  —  cian.snippets { ... }
-- ----------------------------------------------------------------------------
-- シェルへ送る一行に、名前を付けておくもの。`Ctrl+Shift+Enter`（`:snip`）で
-- 選ぶと、内蔵シェルへ送られます。**設定画面（右クリック → 設定… →
-- スニペット）からも足せます。**
--
--   name    ピッカーに出る名前。省くとコマンドそのものが名前になります
--   cmd     送る一行
--   enter   送ってすぐ走らせるか（既定 true）。false なら打ち込むだけで、
--           目で確かめてから自分で Enter を押せます
--   confirm 送る前に訊くか（既定 false）。間違って走らせたくないものに
--
-- cian.snippets {
--   { name = "ログを追う", cmd = "tail -f /var/log/messages" },
--   { name = "空き容量",   cmd = "df -h" },
