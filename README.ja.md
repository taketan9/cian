...
**git と svn はそのまま動きます。** 各行にバッジ（`●` ステージ済み、`✚` 変更、`?` 未追跡、`‼` 競合）、ステータス行にブランチ（または `svn r123`）、F3 は HEAD との差分行に印。`:stage` `:unstage` `:discard` `:gitlog`（`:svnlog`）`:gitdiff`、ビューアの `B` で blame — すべて右クリック **Git ▸** / **SVN ▸** にもあります。cian はあなたの `git` / `svn` を呼びます。
