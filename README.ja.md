...
**`:%!` はファイルの文字コードのまま渡します。** Shift_JIS のファイルは Shift_JIS のまま外部コマンドへ行き、返ってきたものも同じ文字コードとして読みます ── Windows の `sort` や `findstr` はその文字コードを期待するからです（UTF-16 だけは例外で、標準入力で読めるものがほとんど無いので UTF-8 で渡します）。シェルは下のシェルパネルと同じものなので、ここで書いたパイプは下で書いたときと同じに動きます。**失敗したときは何も変えません** ── 終了コードが 0 でなければ、コマンドの言い分を状態行に出して、中身はそのままにします。
