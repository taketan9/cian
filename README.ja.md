...
- **操作はキューに並びます。** コピー中の2件目は拒否されず順番待ちに。`:queue` で実行中と待ち行列、`x` で停止・取り消し。転送失敗は2回まで自動再試行。30秒バイトが動かない転送は `⚠ 停滞` になり、もう一度 `x` で応答しない処理を強制終了して、残りを流します。
- **長い処理は終了時にベルと通知**を出します。トグル（`T`）か `cian.set_option("notify", false)` でオフ。

**`:` の行。** `←` `→` `Home` `End` でカーソルが動き、`↑` `↓` でこの起動で打った行を辿れます（失敗した行も残ります）。`Tab` は、行頭ならコマンド名を、それ以外ならパスを**候補が一致するところまで**補って、複数あるときは名前を並べます ── 勝手に1つ選ぶことはしません。`Ctrl+U` で行を消し、`Ctrl+V` で貼り付けます。
...
| **`T`**（`:toggle`） | ライブ設定 — 隠しファイル・シンクロ入力・通知・転送ベリファイ・言語 |
...
| `:%!cmd` / `:.!cmd` / `:'<,'>!cmd` | ファイル全体／この行／選択範囲を外部コマンドに通して、返ってきたもので置き換えます。`:%!sort`、`:.!date`、`:'<,'>!tr a-z A-Z` |
| `Ctrl+R` | やり直し |
...
**`:%!` はファイルの文字コードのまま渡します。** Shift_JIS のファイルは Shift_JIS のまま外部コマンドへ行き、返ってきたものも同じ文字コードとして読みます ── Windows の `sort` や `findstr` はその文字コードを期待するからです（UTF-16 だけは例外で、標準入力で読めるものがほとんど無いので UTF-8 で渡します）。シェルは下のシェルパネルと同じものなので、ここで書いたパイプは下で書いたときと同じに動きます。**失敗したときは何も変えません** ── 終了コードが 0 でなければ、コマンドの言い分を状態行に出して、中身はそのままにします。なお、コマンド側がバイト列を拒むことはあります ── Mac の `tr` は UTF-8 ロケールで Shift_JIS のバイトを「Illegal byte sequence」と断ります。それはそのコマンドの都合で、cian は断られたことをそのまま伝えます（`LC_ALL=C tr …` のように書けば通ります）。
...
notepad のキー操作では、開いた瞬間から入力状態です。Shift+矢印で選択、Alt+Shift で矩形、
...
- **カーソル追従プレビュー（既定オン）。** シェル枠の領域がカーソル位置のものをプレビューします — コードは色つき、画像、ディレクトリとアーカイブの一覧、Office/PDF のテキスト。シェルは下で動き続けていて、`Shift+J` かクリックで戻ります。リモートペインではあえてプレビューしません（触れるたびダウンロードになるため）。画像は端末のプロトコル（kitty / iTerm2 / sixel）があればそれで、無ければ半角ブロックで描きます。プロトコルを名乗るのに何も描かない端末では `:image` で切り替えられ、`:log` が現在の方式を表示します。`:preview` でオフ。展開やスキャンをしないと何も出せない種類は**既定で対象外**です — `.vsix` `.jar` `.whl` `.iso` `.msi` `.pdf` `.mdf` など。`.zip` や `.tar` は対象外にしていません（中を見るために開くので）。`cian.set_option("preview_skip", { "bak" })` で追加できます。`F3` ではどれも開けます。
...
| **`:mask *.log`** | **付けたまま歩くフィルタ** ── `/` と違い、ディレクトリを移っても効き続けます。`*.log *.trc` のように並べれば「どれか」、`/正規表現/` も同じ書き方。`:mask` だけで解除 |
| **`f`** | このディレクトリ内の一致を順に移動 |
| **`Shift+F`**（`:find`） | このディレクトリ以下から名前で検索 |
...
| **`:mirror`** | **二画面の同期移動** ── 片方をディレクトリへ入れると、もう片方も自分の側の同じ名前へ。上へも同じだけ上がります。向こうに無いときは、そう言って止まります |

**`/` とマスクは別のものです。** `/` は「いま見ているものを絞る」で、ディレクトリを移れば消えます。マスクは「今日はログを見ている」で、外すまで付いて回ります。**ディレクトリはマスクに関係なく出ます** ── 通り道を隠すマスクは、動くために外さなければならないマスクだからです。効いている間は状態行に `マスク *.log (3/12 件)` と出ます。
...
| `:readonly` | 読み取り専用属性の切替（`:readonly on` / `off` で確実に） |
| `:touch` | 選んでいるものの日時をいまに（ディレクトリも）。`:touch <名前>` はそのファイルを作ります |
| `:hash md5` / `:hash sha256` | チェックサム |
| `:count` | ファイル数・行数・ステップ数 |
| `:log` | この版・どの設定をどこから読んでいるか・端末の素性を1枚で（`:where` は設定の場所だけを出す従来の形） |
...
**git と svn はそのまま動きます。** 各行にバッジ（`●` ステージ済み、`✚` 変更、`?` 未追跡、`‼` 競合）、ステータス行にブランチ（または `svn r123`）、F3 は HEAD との差分行に印。ビューアの `B` で blame。すべて右クリック **Git ▸** / **SVN ▸** にもあります。cian はあなたの `git` / `svn` を呼びます。

**操作の名前は片方だけです。** どちらのリポジトリかはディレクトリが知っているので、`:stage` `:unstage` `:discard` `:commit` の4語が両方で同じように効きます（`:svnadd` のような古い名前も通りますが、一覧には出しません）。見るほうの2つだけは名前に `git` / `svn` が入ります ── `:gitlog` / `:svnlog`、`:gitdiff` / `:svndiff`。`:update` と `:resolve` は svn にしかない操作なので svn 専用で、git のディレクトリで叩くと理由を言って断ります（git の `pull` は merge を伴うので、cian には入れていません）。

**`:commit` だけは、同じ語でも行き先が違います。** git は手元のリポジトリに記録して終わり、svn は成功した瞬間にサーバへ届きます。だから**訊く画面のほうがどちらか言います** ── git は「手元のリポジトリに記録します」、svn は「コミットするとサーバに届きます」。
...
**鍵はパスワードより先に試します。** `key`（あるいは `password` / `password_cmd`）を
書いたユーザは、パスワードを訊かれずに繋ぎます ── `ssh` 自身と同じ順序です。欄が
出るのは、そのユーザに何も設定していないときと、**鍵がそのサーバに断られたとき**
だけで、後者はどの鍵が断られたかを言います。
...
下段は本物のシェル（`$SHELL`）です。**`Shift+J`**・クリック・`:shell` でフォーカス、**Esc** でファイルへ戻ります。**流れていった出力は残ります** — 10,000行、色もそのまま。パネル上でホイールを回すとさかのぼれ、`Shift+PageUp` / `Shift+PageDown` で1画面ずつ、`Shift+↑` / `Shift+↓` で1行ずつ、`Shift+Home` / `Shift+End` で両端へ。今どこを見ているかはバッジとバーが示し、何か入力すれば最新に戻ります。分割している場合は各ペインが独立してさかのぼれます。全画面プログラム（vim, less, htop）は Esc とファンクションキーを自分のものにします。ドラッグで範囲選択（離すとコピー）。**マウスを使わずに選ぶなら `Shift+←` / `Shift+→`** — シェル自身のカーソルから1文字ずつ伸び、行をまたいで折り返すので、上の出力まで届きます（`Ctrl+C` でコピー、`Esc` で解除してもシェルに留まります）。右クリックで専用メニュー — SSH接続・貼り付け・セッションログ・SFTP/SCP・文字コード。
...
**シンクロ入力** は `:sync` か右クリックから。1回打てばタブ内の全ペインに届き、その間パネルは明るい **⇄ SYNC** 枠になります。
...
**Ctrl+Shift+Enter**（`:snip`）でピッカー。入力で絞り込み、Enter で送信。`enter = false` は入力だけして確認を待ち、`confirm = true` は先に聞きます。
...
`cian-tui --macro thing.lua` で起動時に実行（`.lua` を `cian-tui.exe` に関連付ければダブルクリックで実行できます）、`--macro-name "…"` は設定内のマクロを名前で実行します。
...
**入口は2つです**（2026-09-20 に6つから絞りました）。`:ai` が訊くほうで、`:aicmd` がシェル向け。

| コマンド | できること |
|---|---|
| `:ai <訊きたいこと>` | チャット。質問は会話欄に入るだけで、送るのは Enter を押したとき |
| `:ai commit` | ステージ済み差分からコミットメッセージ |
| `:ai error` | 直前のシェルエラーを説明 |
| `:ai diff` | 画面の差分を説明（差分表示中の `x` でも） |
| `:ai log` | 選択したログを診断。エラー・時系列・原因の見当 |
| `:aicmd <やりたいこと>` | 今いるシェル（ローカル、または SSH 先）向けのコマンドを下書き。実行はしません |
| F3 の `S` | 読んでいるファイルを要約 |

古い名前（`:aicommit` `:aierror` `:aidiff` `:ailog`）も通りますが、一覧には出しません。**そして `cian.ai{…}` を書いていない機械では、コマンドパレットにも右クリックにも AI は出ません** ── 打てば「未設定です」と答えるので、道は塞いでいません。

**会話は続きます。ウィンドウ版にも会話窓が入りました**（`:ai`／要約／エラー説明／差分説明／ログ診断／選択範囲の三つ、全部そこに出ます。`Ctrl+R` で過去の会話、`Esc` で中断、画像はそのまま貼れます）。**履歴は両前端で同じ `ai_history.json`** ── 前に訊いたことを探すのに、どちらのビルドだったかを思い出す必要はありません。それまでのやり取りを一緒に送ります — 「さっきのやつ」が通じるということです。長くなると**古い手から丸ごと**落とし（半端な手は話者のいない文なので）、**最新の手だけは重くても必ず送ります** — それが次の質問の対象だからです。（かつては構造化した依頼 ── ゴミ候補・構成案・リネーム案・意味検索 ── も並んでいましたが、2026-09-06 に落としました。どれも「ファイルへの操作を提案する」もので、決定的でオフラインな `:duplicate` `:renamepattern` `:renamelist` と同じ仕事を、当てにならないやり方で二重に持っていたからです。残したのは**訊くと答える**ものだけです。）
...
IME が変換中の英字は、そもそも cian に届きません（確定するまで端末が持っています）。だから単キーのコマンドは IME を切るまで反応しません。届いていないキーは拾えません。
...
**書体 — `face`。ウィンドウ版だけの設定です。** 端末版の書体はエミュレータのものなので、サイズと同じ理由で cian からは触れません。ウィンドウ版は Chromium で描いていて、教えられます:
...
stable の Rust だけで通ります（残りは cargo が取ってきます）。ウィンドウ版は
...
- **管理者で起動しているのにコピーが拒否される（共有環境・UNC）。** 管理者グループに入っていることと、そのフォルダの ACL があなたを許可していることは別の話です。ACL が許可していなければ、昇格し直しても同じトークンが同じ壁に当たります。

  管理者が持っているのは `SeBackupPrivilege` / `SeRestorePrivilege` で、これは **ACL を素通りして読み書きする**ための特権です。`c` / `m` が拒否されると cian が訊きます ── **「管理者の権限で robocopy を実行してやり直しますか？」**。**はい**を押したときだけ `robocopy /B` が走り、**権限は一切変更しません**。エクスプローラが毎回同じことを訊いてきて、フォルダの権限が後から見ても変わっていないなら、それはこちらの動きです。robocopy でも通らなかったときにだけ、2枚目として「昇格してやり直しますか？」が出ます ── 昇格が直せる唯一のこと（cian を管理者で起動していない）は、robocopy がその場で言ってくれるからです。

  cian は ACL そのものを書き換えません。他の人がサインインしている機械では、それはあなただけでなく**その全員に対する変更**だからです。

  **特権はその機械の中の話です。** `\\別ホスト\share` の可否は向こうの機械の ACL とそこでのあなたの身元が決めるので、向こうでも管理者でなければ通りません。
- **困ったら `:log`。** この版・設定の在り処・端末の素性を1枚で出し、送れるようにファイルにも書きます。診断ログは `:log on` でその場から取れます（`:log <パス>` で場所指定、`:log off` で停止、起動時からなら `CIAN_LOG=/tmp/cian.log`）。パニック時も端末を復旧してから落ちるので、`reset` が必要になることはありません。
