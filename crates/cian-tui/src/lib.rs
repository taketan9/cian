...
impl MenuItem {
    fn label(self, lang: Lang) -> &'static str {
        match self {
            ...
            MenuItem::TogglesMenu => tr(lang, "Switches…  (T)", "トグル…  (T)"),
            ...
            MenuItem::SyncStart => tr(lang, "Synchronize input  ⇄", "シンクロ入力を開始  ⇄"),
            MenuItem::SyncStop => tr(lang, "Stop synchronize  ⇄", "シンクロ入力を停止  ⇄"),
            MenuItem::SyncMember => tr(lang, "Toggle this pane in sync group  ⇄", "このペインをシンクロ入力に含める/外す  ⇄"),
    ...
    vec![
        ...
        (
            ...
            vec![
                ...
                entry("T", None, "UI toggles menu: dotfiles, input sync, notifications… (also :toggle)", "トグル：隠しファイル/シンクロ入力/通知…（:toggle でも）"),
        ...
        (
            ...
            vec![
                ...
                entry(":commit", None, "commit with a message you write. git records it here, svn sends it to the server", "自分で書いたメッセージでコミット。git は手元に記録、svn はサーバに届きます"),
                entry(":discard", None, "git/svn: throw away worktree changes (git checkout / svn revert)", "作業ツリーの変更を破棄（git checkout / svn revert）"),
                entry(":gitlog / :svnlog", None, "commit log / a file's history — git or svn (also right-click)", "コミットログ／ファイル履歴 — git・svn（右クリックでも）"),
                entry(":gitdiff", None, "the selected file's diff vs HEAD/BASE — git or svn", "選択ファイルの HEAD／BASE との差分 — git・svn"),
                entry(":update", None, "svn update the working copy. svn only (also right-click SVN ▸)", "svn update で作業コピーを更新。svn のみ（右クリック SVN ▸ でも）"),
                entry(":resolve", None, "svn resolve --accept working. svn only (mark conflicts resolved)", "svn resolve --accept working。svn のみ（競合を解決）"),
        ...
        (
            ...
            vec![
                ...
                entry("  keys, spelled as words", None, ":branch (b)  :bookmark (s)  :visual (v)  :zoom (F12)  :forward (Alt+→)  :local (Esc, from a server)  :tab / :tabclose", "キーと同じものを名前でも: :branch (b)  :bookmark (s)  :visual (v)  :zoom (F12)  :forward (Alt+→)  :local (サーバから戻る Esc)  :tab / :tabclose"),
                entry("  :revealos", None, "show the file in Finder / Explorer (:showinfinder is the same)", "Finder / エクスプローラで表示（:showinfinder も同じ）"),
                entry("  :diffedit / :lsar / :filelog", None, "compare the two panes, list an archive, this file's history — the same as `=`, Enter on an archive, and :gitlog", "左右を比較／アーカイブの中身／このファイルの履歴 ── `=`・アーカイブで Enter・:gitlog と同じ"),
                entry("  :settings", None, "the window build's settings screen. in a terminal, edit init.lua (`:log` says where)", "ウィンドウ版の設定画面。端末版では init.lua を直接（場所は `:log`）"),
                ...
                entry(":aicmd", None, "AI: shell command from a description. it drafts, you run it", "AI: 説明からシェルコマンド。下書きするだけで、実行はしません"),
        ...
        (
            ...
            vec![
                ...
                entry("Shift+F12", None, "zoom active split pane (toggle)", "アクティブな分割ペインをズーム（もう一度で戻る）"),
                entry(":shellname", None, "name this shell tab for what it is doing (empty puts the number back)", "このシェルタブに用途の名前を付ける（空で番号に戻る）"),
                entry(":sync", None, "synchronize: type into all panes at once (also right-click)", "シンクロ入力：全ペインへ一括入力（右クリックでも）"),
