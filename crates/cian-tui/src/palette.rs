...
pub(crate) fn command_list() -> &'static [(&'static str, (&'static str, &'static str), bool)] {
    &[
        ...
        ("stage",      ("stage the selection (git or svn)", "選択をステージ（git・svn）"), false),
        ("unstage",    ("unstage the selection (git or svn)", "選択をアンステージ（git・svn）"), false),
        ...
        ("update",     ("svn update the working copy (svn only)", "svn update で作業コピーを更新（svn のみ）"), false),
        ...
        ("toggle",    ("UI toggles menu (T)", "トグル（T）"), false),
